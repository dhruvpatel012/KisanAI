...
- **Day 12**: Configured and deployed cloud production architecture (FastAPI backend on Render.com, React frontend on Vercel.com, whitelisted MongoDB Atlas access). Implemented 15-minute thread-safe backend weather caching and direct frontend client Open-Meteo fetching to bypass cloud IP rate limits, adding client-side reverse geocoding city lookup (Nominatim). Migrated crop scan upload storage from ephemeral server disk files to persistent Base64 Data URLs inside MongoDB Atlas, eliminating disk dependency and preserving scan histories permanently.
